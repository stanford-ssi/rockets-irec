function [velocities,heights,n] = terminal_velocity_fun(rocket,dt)
...
while (h_curr > 10)
    ...
    F_g = rocket.drymass.*lbs2kg * g0;
    if h_curr > rocket.main.deploy_h*in2m
        F_drag = .5 * rho(round(h_curr)) * v_curr^2 * rocket.drogue.S * in22m2 * rocket.drogue.Cd;
    else
        F_drag = .5 * rho(round(h_curr)) * v_curr^2 * rocket.main.S * in22m2 * rocket.main.Cd;
    end
    F_curr = F_drag - F_g;
    
    % New change in velocity is specific force * small change in time
    % Add to current velocity
    dv = (F_curr/rocket.drymass)*dt;
    v_curr = v_i + dv;
